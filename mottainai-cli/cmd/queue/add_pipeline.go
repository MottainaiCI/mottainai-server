...
func newQueueAddPipelineInWaitingCommand(config *setting.Config) *cobra.Command {
	var cmd = &cobra.Command{
		Use:   "add-pipeline [OPTIONS]",
		Short: "Add pipeline in waiting to a queue (dev only)",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			req := &schema.Request{
				Route: v1.Schema.GetQueueRoute("add_pipeline"),
