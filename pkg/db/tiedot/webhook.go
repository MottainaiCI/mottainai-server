...
func (d *Database) GetWebHooksByField(field, name string) ([]webhook.WebHook, error) {
	...
	queryResult, err := d.FindDoc(WebHookColl, `[{"eq": "`+name+`", "in": ["`+field+`"]}]`)
	if err != nil {
		return res, err
	}
...
func (d *Database) AllWebHooks() []webhook.WebHook {
	WebHooks := d.DB().Use(WebHookColl)
	WebHooks_id := make([]webhook.WebHook, 0)

	WebHooks.ForEachDoc(func(id int, docContent []byte) (willMoveOn bool) {
		t := webhook.NewWebHookFromJson(docContent)
